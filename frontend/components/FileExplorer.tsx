...
function TreeItem({
  ...
  if (node.type === "dir") {
    return (
      <div>
        ...
        >
          <span
            style={{ display: "inline-block", transform: open ? "rotate(90deg)" : "rotate(0deg)", fontSize: "10px" }}
            className="text-[var(--color-text-muted)] w-3 shrink-0 transition-transform duration-150"
          >
            {">"}
          </span>
          <span className="text-xs font-medium text-[var(--color-text)] truncate flex-1">{node.name}</span>
        </button>
        {open && node.children && (
          <div>
            {node.children.map((c) => (
  ...
  return (
    ...
    >
      <span
        className="text-[8px] px-0.5 rounded shrink-0 font-mono"
        style={{ backgroundColor: langColor + "33", color: langColor }}
      >
        {LANG_ICONS[node.language ?? "unknown"] ?? "FILE"}
      </span>
      <span className="text-xs text-[var(--color-text-muted)] truncate flex-1 group-hover:text-[var(--color-text)] transition-colors">
        {node.name}
      </span>
      {node.chunk_count !== undefined && (
        <span
          className="text-[8px] px-1 py-0.5 rounded shrink-0 opacity-60"
          style={{ backgroundColor: langColor + "33", color: langColor }}
        >
          {node.chunk_count}
        </span>
...
export default function FileExplorer({ repoId, onFileSelect, selectedFile }: FileExplorerProps) {
  ...
  const collectFiles = useCallback(
    (nodes: TreeNode[]) => {
      for (const n of nodes) {
        if (n.type === "file")
          flatFiles.push({ file_path: n.path, language: n.language ?? "unknown", chunk_count: n.chunk_count ?? 0 });
  ...
  useEffect(() => {
    ...
    getRepoFiles(repoId)
      .then((d) => {
        ...
        setStats({ files: d.total_files, chunks: d.total_chunks, langs: d.languages });
      })
  ...
  if (collapsed) {
    return (
      ...
      >
        <span style={{ fontSize: "10px", color: "var(--color-text-muted)" }}>F</span>
      </button>
    );
  }
  ...
  return (
    <aside
      className="w-56 shrink-0 border-r border-[var(--color-border)] flex flex-col h-full overflow-hidden"
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--color-border)] shrink-0">
        <div className="flex items-center gap-1.5">
          ...
          {stats && (
            <span className="text-[9px] text-[var(--color-text-muted)] bg-white/5 px-1 rounded">
              {stats.files}
            </span>
          )}
        </div>
          ...
          <div className="flex gap-1 flex-wrap">
            {stats.langs.slice(0, 4).map((lang) => (
              <span
                ...
                style={{
                  backgroundColor: (LANG_COLORS[lang] ?? "#6e7681") + "25",
            ...
            {stats.langs.length > 4 && (
              <span className="text-[9px] text-[var(--color-text-muted)]">+{stats.langs.length - 4}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="px-2 py-1.5 border-b border-[var(--color-border)] shrink-0">
        <input
          ...
          className="w-full bg-white/5 border border-[var(--color-border)] rounded text-[10px] text-[var(--color-text)] placeholder-[var(--color-text-muted)] px-2 py-1 outline-none focus:border-[var(--color-accent)]/50 transition-colors"
        />
      </div>
      ...
      {selectedFile && (
        ...
        >
          <span className="text-[9px] text-[var(--color-accent)] truncate flex-1">
            Filter: {selectedFile.split("/").pop()}
          </span>
          <button
            onClick={() => onFileSelect(null)}
            className="text-[var(--color-accent)] hover:text-[var(--color-text)] transition-colors ml-1 shrink-0"
          >
      ...
      <div className="flex-1 overflow-y-auto py-1">
        ...
        {error && <div className="px-3 py-2 text-[10px]" style={{ color: "#f87171" }}>{error}</div>}

        {!loading && !error && filteredFiles && (
          <div>
            {filteredFiles.length === 0 && (
              <div className="px-3 py-2 text-[10px] text-[var(--color-text-muted)]">No files match</div>
            )}
            {filteredFiles.map((f) => {
              ...
              return (
                <button
                  key={f.file_path}
                  onClick={() => onFileSelect(isSel ? null : f.file_path)}
                  className={`w-full flex items-center gap-1.5 py-[3px] px-2 text-left rounded transition-all ${
                    isSel ? "bg-[var(--color-accent)]/15" : "hover:bg-white/5"
                  }`}
                  title={f.file_path}
                >
                  <span className="text-[8px] px-0.5 rounded shrink-0 font-mono" style={{ backgroundColor: lc + "33", color: lc }}>
                    {li}
                  </span>
                  <span className="text-xs text-[var(--color-text-muted)] truncate flex-1">{name}</span>
                  <span className="text-[8px] px-1 py-0.5 rounded shrink-0" style={{ backgroundColor: lc + "33", color: lc }}>
                    {f.chunk_count}
                  </span>
                </button>
              );
        ...
        {!loading && !error && !filteredFiles && (
          <div>
            {tree.map((n) => (
              <TreeItem key={n.path} node={n} depth={0} selectedPath={selectedFile} onSelect={onFileSelect} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="px-3 py-1.5 border-t border-[var(--color-border)] shrink-0">
        <p className="text-[8px] text-[var(--color-text-muted)] leading-relaxed">
          Click a file to filter answers to that file only.
        </p>
      </div>
    </aside>
  );
